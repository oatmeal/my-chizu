...
- **Screenshot photo layer** — built for the 280 screenshots whose filename carries a world coordinate (263 overworld, 17 nether). Layer, clustering, lightbox, photos pane and timeline filtering all exist, and have had one review pass against a live preview (the switch moved into the photos tab, pins gained a guaranteed minimum separation, pin↔row hover linking, per-date 📷 counts, split-cluster badges, a lightbox filmstrip, photo permalinks with a copy button in the lightbox). **Phase 3** is also done: photo review lives in the `mc-screenshot-to-map` web app, reusing its decision states and digest rule, and the first pass through it approved 265 of 280. Remaining, per `PLAN_PHOTOS.md`: **Phase 4**, OCR of the coordinate HUD and taskbar clock, which brings in the 214 descriptively-named screenshots and gives every photo a wall-clock time and a real `y`. Phase 3 was deliberately taken before Phase 4 — OCR is what produces review work at volume, and building the reviewer second would have meant hand-editing a CSV against 214 rows. A screenshot that is nothing but a chat log is out of scope and never published; chat visible incidentally behind a photograph of a place is fine.

- **Photos by other people** — publishing the screenshots two members posted to the server's Discord, credited to whoever took them, with consent gating the pipeline rather than being remembered. Designed in `PLAN_CONTRIBUTORS.md`; not started. 195 attachments from the two who have agreed, of which HUD OCR places 82. Shares its OCR component with the photo layer's Phase 4, and needs a map placement UI in `mc-screenshot-to-map`'s review app for the rest.
  ...
  The first photo set is committed to `llmr` — 265 photos, 530 files, plus both `photos.json` layer files. The encoding is therefore settled and should not be revisited casually: each re-encode adds another ~35 MB of blobs to that repo's history permanently, and it invalidates every approval, because a decision's digest covers the encoded bytes. A change can still be previewed end-to-end without committing anything (`llmr/deploy/` is gitignored) — see "Previewing before you commit" in `PLAN_PHOTOS.md`.
