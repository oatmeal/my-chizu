...
import { hashLayerIds } from "./photos.js";
import {
  DIM_OVERWORLD,
  DIM_NETHER,
  DIM_END,
...
import { setupPhotoPanel } from "./setupPhotos.js";
...
async function changeDim(mymap, newDim) {
  ...
  mymap.hashObj.dD[mymap.dim] = {
    ...
    // Photo layers are left out: their visibility is the top-level `pv`, and a
    // build-assigned layer id is not a public interface. See `hashLayerIds`.
    v: hashLayerIds(mymap.dimData.visibleLayers, mymap.dimData.layers),
    h: {
      d: mymap.dimData.timeline.date,
      e: mymap.dimData.timeline.exact,
      f: mymap.dimData.timeline.fill,
      p: mymap.dimData.timeline.photoDate,
    },
  };
  ...
  for (const id of mymap.dimData.visibleLayers) {
    ...
    // A photo layer has no checkbox of its own -- the photos tab owns it -- so
    // the panel bookkeeping is skipped and only the layer itself goes on.
    const { check } = mymap.layerCache[id];
    if (check) check.checked = true;
    await ensureLayerLoaded(mymap, id);
    if (check) {
      check.parentElement.style.backgroundColor = `hsla(${
    ...
    mymap.dimCache[dim] = {
      ...
      visibleLayers: new Set(
        (Array.isArray(
          ...
          (dim === DIM_NETHER
            ? // default to showing all layers in the nether -- except the photo
              // layer, whose visibility belongs to the photos tab
              layers.filter(({ kind }) => kind !== "photos").map(({ id }) => id)
            : [])
      ),
    };
...
async function init(mapid) {
  ...
  for (const id of dimRadioIds) {
    ...
    el.onchange = async () => {
      ...
      mymap.hashObj.dD[mymap.dim].v =
        hashLayerIds(mymap.dimData.visibleLayers, mymap.dimData.layers);
      // TODO: other settings?
      mymap.updateHash();
  ...
  setupPhotoPanel(mymap, ensureLayerLoaded);
