 ...
 * count rather than folding the future in (`3+2`); an accent drawn over the
 * whole stack would be the one part of the pin still claiming otherwise. A
 * wholly future cluster falls back to all of them, because an accent of nothing
 * is not an improvement on one that is dimmed along with the pin.
 *
 * Ordered by id, not by count. The renderer reorders by the site's stated
 * `authorOrder` where there is one, but a count is not a place to decide it:
 * what matters either way is that a person sits in the same place on every pin.
 *
 ...
 * greys.
 *
 * This is the fallback, not the ruling: `site.json`'s `photos.authorColors`
 * names a colour for anyone the palette should not choose for.
 */
const AUTHOR_COLORS = [
...
/**
 * Where the site wants an author to sit, smaller first.
 *
 * `site.json`'s `photos.authorOrder` is a list of ids, and everybody it does
 * not name shares one rank at the end, so the rules that were there before it
 * -- share of the dimension for the chips, id for a pin's segments -- break the
 * tie among the rest. A partial list is therefore a legitimate thing to write:
 * "these two first, then whatever you were doing".
 *
 * It lives in `site.json` rather than beside the names in the layer file
 * because it is not a fact about a person. `authors` is rewritten wholesale by
 * the publishing repo from its consent record; this is the site owner saying
 * how they want their pane read, and it survives that rewrite.
 *
 * @param {{authorOrder?: number[]}} [config]
 * @param {number} [by]
 * @returns {number}
 */
export function authorRank(config, by) {
  const at = config?.authorOrder?.indexOf(by) ?? -1;
 ...
 *
 * `site.json`'s `photos.authorColors` overrides the palette for the ids it
 * names, because which of six hues an author gets is a look, and the palette
 * decides it by arithmetic on an id that was handed out in the order people
 * gave permission. The gate above still rules: an override does not put a
 * colour on a map with one contributor, where there is nothing to tell apart.
 *
 * @param {Object<string, {name: string}>} [authors]
 * @param {number} [by]
 * @param {{authorColors?: Object<string, string>}} [config]
 * @returns {string|undefined}
 */
export function authorColor(authors, by, config) {
  if (!authors || Object.keys(authors).length < 2 || !authors[by]) return undefined;
  return config?.authorColors?.[by] || AUTHOR_COLORS[(by - 1) % AUTHOR_COLORS.length];
}
 ...
 *
 * Ordered by `photos.authorOrder` where the site states one, and by count
 * largest first where it does not -- that being the order somebody scanning for
 * a name wants; ties fall back to the id so it is stable. The two compose:
 * a partial `authorOrder` fixes the people it names and leaves the rest sorted
 * by share, which is the only reading of a partial list that is not a surprise.
 *
 ...
 * @param {Object<string, {name: string}>} [authors]
 * @param {{authorOrder?: number[], authorColors?: Object<string, string>}} [config]
 * @returns {Array<{id: number, name: string, count: number, color: string|undefined}>}
 */
export function authorCounts(photos, authors, config) {
  const counts = new Map();
  for (const photo of photos) {
  ...
  return [...counts.entries()]
    .map(([id, count]) => ({
      ...
      count,
      color: authorColor(authors, id, config),
    }))
    .sort((a, b) =>
      authorRank(config, a.id) - authorRank(config, b.id)
      || b.count - a.count
      || a.id - b.id);
}
...
/**
 * Whether an arriving hash asks for photos to stay on the map.
 *
 * `pv` is the setting itself: one flag for the whole map, because that is what
 * the checkbox is -- it outlives a dimension switch, so filing it per dimension
 * would promise a per-dimension thing that does not exist. Any truthy JSON
 * value counts, so `pv=1` and `pv=true` both read; `1` is what gets written,
 * being the shortest thing that survives `JSON.parse`.
 *
 * **The `v` fallback is the old spelling and is read, never written.** Before
 * `pv` the state rode the visible-layer list as the photo layer's id, which
 * worked but published a build-assigned number as the public interface for a
 * checkbox. Links with it in the wild keep opening the map they described.
 *
 * @param {{pv?: unknown, ph?: unknown}} [hashObj]
 * @param {number[]} [photoIds] - the photo layer ids of the arrival dimension
 * @param {Set<number|string>} [visibleLayers] - as seeded from `dD[dim].v`
 * @returns {boolean}
 */
export function hashPhotosVisible(hashObj, photoIds = [], visibleLayers) {
  if (hashObj?.pv) return true;
  return photoIds.some((id) => visibleLayers?.has(id));
}

/**
 * The layer ids that belong in the hash's `v` for a dimension.
 *
 * Every visible layer except the photo layers, whose visibility is `pv` and is
 * not a layer at all as far as a URL is concerned. `visibleLayers` still holds
 * them -- it is what the restore loop and the layers panel run on -- so this is
 * the boundary between an internal set and a published one, and it is the only
 * place the two differ.
 *
 * @param {Iterable<number|string>} visibleLayers
 * @param {Array<{id: number, kind?: string}>} [layers] - the dimension's layers
 * @returns {Array<number|string>}
 */
export function hashLayerIds(visibleLayers, layers) {
  const photo = new Set(
    (layers || []).filter((layer) => layer.kind === "photos").map((layer) => layer.id)
  );
  return Array.from(visibleLayers ?? []).filter((id) => !photo.has(id));
}
