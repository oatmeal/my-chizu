...
import { renderPhotos } from "./setupPhotos.js";
...
const RENDERERS = [
  ...
  { key: "photos", render: renderPhotos },
];
...
export function setupLayerPanel(mymap) {
  ...
  function initLayers(layerList) {
    ...
    for (const layer of layerList) {
      // Photo layers deliberately get no row here: they are switched on by
      // opening the photos tab, because photos are a way of looking at the map
      // rather than one overlay among many -- see `setupPhotoPanel`. They still
      // need a cache entry, which is what that tab loads them through.
      ...
      eye.onclick = async function () {
        ...
        fitAll.onclick = function () {
          // A layer can legitimately be empty right now -- a photo layer holds
          // nothing when the timeline is scrubbed back before its first photo --
          // and fitting to empty bounds throws.
          const bounds = dataLayer.getBounds();
          if (bounds.isValid()) mymap.fitBounds(bounds, { animate: true });
        };
