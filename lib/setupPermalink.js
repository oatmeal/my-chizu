...
export function setupPermalinkPanel(mymap) {
  ...
  mymap.sidebar.on("content", function (e) {
    if (e.id === "link") {
      ...
      // Photo layers excluded: the photos tab owns their visibility and it
      // rides the top-level `pv`. See `hashLayerIds`.
      mymap.hashObj.dD[mymap.dim].v = hashLayerIds(
      ...
      mymap.hashObj.dD[mymap.dim].h.p = mymap.dimData.timeline.photoDate;
      // TODO: other settings?
      updatePermalink();
    } else {
