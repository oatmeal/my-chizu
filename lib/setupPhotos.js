...
/**
 * The names behind every `by`, merged across layers.
 *
 * A layer file lists only the authors it credits, so the overworld's block and
 * the end's are different subsets of the same people. They are merged rather
 * than read per layer for two reasons: the lightbox is built once and outlives
 * any one layer, and whether the accent colours exist at all is "is more than
 * one person on this map" -- a question that must not answer itself differently
 * in the nether.
 *
 * Merging alone did not settle that second one, because what had been merged
 * depended on where the visitor had been. `dimData.photoAuthors` is the build's
 * union over every layer file in every dimension (see `collectPhotoAuthors` in
 * `build-data.mjs`), so it is the same cast in each dimension and arrives
 * before any layer does. Taking it first is what makes a person's colour, and
 * whether anyone has one, a property of the map rather than of the route
 * through it.
 *
 * The per-layer merge stays underneath it: a data repo built by an older engine
 * carries no union, and it has to keep working exactly as it did.
 */
function rememberAuthors(mymap, data) {
  const wholeSite = mymap.dimData?.photoAuthors;
  if (wholeSite) mymap.photoAuthors = { ...mymap.photoAuthors, ...wholeSite };
 ...
 *
 * Null rather than "every id" so the untouched case costs nothing, and so the
 * hash carries nothing for a map nobody has filtered.
 */
...
/**
 * Set the filter, and say so everywhere that has to agree.
 *
 * Three things move together and there is no case where one should move
 * alone: the pane's own state, the hash the permalink is assembled from, and
 * the event that redraws the pins. Routing every change through here is what
 * stops a copied link describing a map the visitor is not looking at.
 *
 * `null` is everybody and writes no hash key at all; the empty set is every
 * author switched off, which is a real state and is carried as `by=[]`.
 */
function setAuthorFilter(mymap, allowed) {
  mymap.photoAuthorFilter = allowed;
  if (allowed) mymap.hashObj.by = [...allowed];
  else delete mymap.hashObj.by;
  mymap.fire("photofilterchange");
}

/**
 * The filter an arriving URL asks for, or null where it asks for nothing.
 *
 * Anything that is not a list of numbers is not an answer to "which authors",
 * so it is discarded rather than half-read -- the map then opens unfiltered,
 * which is the state every visitor without a link gets. Ids that name nobody
 * survive parsing: the roster is not loaded yet, and a chip the pane cannot
 * draw simply never matches a photo.
 */
function hashAuthorFilter(hashObj) {
  const by = hashObj?.by;
  if (!Array.isArray(by) || by.some((id) => !Number.isInteger(id))) return null;
  return new Set(by);
}
  ...
  const hashObj = {
    ...
    dD: {
      ...
      [mymap.dim]: {
        ...
        v: hashLayerIds(visibleLayers, mymap.dimData.layers),
        h: {
          d: timeline.date,
          e: timeline.exact,
          f: timeline.fill,
...
/**
 * Whose photos a pin holds, as a CSS background: one colour, or a conic
 * gradient split in proportion. Nothing at all where there is nobody to tell
 * apart, which is `authorColor`'s rule and not a second one.
 *
 * The wedges run in the pane's chip order -- `authorOrder` where the site
 * states one, author id where it does not -- so a person's wedge starts in the
 * same place on every pin and the chips are the legend for it.
 *
 * From `-90deg` so the first wedge starts at twelve o'clock, which is where a
 * reader's eye starts on anything round.
 */
function authorFill(cluster, authors, config) {
  const segments = cluster.authors
    .slice()
    .sort((a, b) => authorRank(config, a.by) - authorRank(config, b.by) || a.by - b.by)
    .map(({ by, count }) => ({ color: authorColor(authors, by, config), count }))
  ...
  const stops = segments.map((segment) => {
    ...
    acc += segment.count;
    return `${segment.color} ${from.toFixed(2)}% ${((acc / total) * 100).toFixed(2)}%`;
  });
  return `conic-gradient(from -90deg, ${stops.join(", ")})`;
}

/**
 * The count badge on a pin, which is also the badge that says whose.
 *
 * A cluster gets its count -- and a cluster the timeline has split, some of
 * whose photos postdate the selected date, gets the split written out as `3+2`:
 * three photos that exist, two that do not yet. Folding those together is what
 * made the old pins confusing, because a bright pin reading `5` could be mostly
 * future.
 *
 * **The badge carries the authorship, and it is the loudest thing on the pin.**
 * It began as a bar along the foot, and the bar was legible only once you had
 * decided to look at a particular pin; the badge is what the eye finds first,
 * so putting the colour there says whose without asking for a second glance.
 * The fill goes on the badge's *rim*, over a near-black core, because the
 * numeral has to stay readable over whichever hues a cluster happens to hold --
 * white on `#43c59e` is thin, and a data repo picks its own colours anyway.
 * The rim is a wheel, so a mixed cluster still shows its true proportion.
 *
 * **A lone photo has no count to show and still has an author**, so it gets the
 * fill as a plain dot in the badge's place. That is the one case the bar used
 * to cover on its own.
 *
 * Where there is nobody to tell apart the badge is exactly what it always was,
 * red and unadorned: a data repo whose photos are all one person's, or a `by`
 * that resolves to nobody, gets the pin it had before any of this existed.
 */
function countBadgeHtml(cluster, authors, config) {
  const { count, dimCount, dimmed } = cluster;
  const fill = authorFill(cluster, authors, config);
  const shown = !dimmed && dimCount > 0
    ? `${count - dimCount}<span class="photo-pin-count-later">+${dimCount}</span>`
    : count > 1 ? `${count}` : "";
  if (!shown) {
    return fill ? `<span class="photo-pin-author-dot" style="background:${fill}"></span>` : "";
  }
  if (!fill) return `<span class="photo-pin-count">${shown}</span>`;
  return `<span class="photo-pin-count photo-pin-count-author"`
    + ` style="--author-fill:${fill}">${shown}</span>`;
}
...
function clusterMarker(mymap, cluster, openLightbox) {
  const config = photosConfig();
  const { thumb } = photoUrls(cluster.lead, config);
  const icon = L.divIcon({
    className: `photo-pin${cluster.dimmed ? " photo-pin-dim" : ""}`,
    html: `<img src="${thumb}" alt="" loading="lazy">`
      + countBadgeHtml(cluster, mymap.photoAuthors, config),
    iconSize: [56, 56],
    iconAnchor: [28, 28],
  });
...
export function setupPhotoPanel(mymap, ensureLayerLoaded = async () => {}) {
  ...
  async function apply() {
    ...
    // The hash carries the setting, not the layer it switches on. Written here
    // rather than from the checkbox's own handler because every path that
    // changes it lands in `apply` -- the handler, the seed from an arriving
    // link, and a dimension switch that re-applies the preference.
    if (persist) mymap.hashObj.pv = 1;
    else delete mymap.hashObj.pv;
  ...
  async function openHashPhoto() {
    ...
    // Synchronously: `photofilterchange` redraws the pins in the handler, and
    // awaiting `photosApplied` here would be waiting on the promise this
    // function is itself the last step of.
    if (unhide(key)) refresh();
  ...
  /**
   * Let a named photo through a filter that is hiding it.
   *
   * This is the one collision between the two things a link can carry, and it
   * is why the filter was kept out of the hash for as long as it was: `find`
   * searches the pins that were drawn, so a `ph` for a photo the filter hid
   * would land on nothing and the link would silently do nothing at all.
   *
   * The photo wins, because a link that names one is *about* that photo, and
   * the filter is how the sender happened to be reading the map. It wins by
   * the smallest possible margin -- their author is added, the rest of the
   * filter stands -- so the visitor still arrives at the map that was sent,
   * with one more person switched on.
   *
   * Returns whether anything changed, which is the caller's cue to bring the
   * pane back into line with the pins the event has already redrawn.
   */
  function unhide(key) {
    const allowed = authorFilter(mymap);
    if (!allowed) return false;
    for (const { data, dataLayer } of mymap.photoLayers || []) {
      if (!mymap.hasLayer(dataLayer)) continue;
      const photo = data.photos.find((p) => p.f === key);
      if (!photo || photo.by === undefined || allowed.has(photo.by)) continue;
      setAuthorFilter(mymap, new Set([...allowed, photo.by]));
      return true;
    }
    return false;
  }
  ...
  function renderAuthors(inView) {
    if (!authorsEl) return;
    const config = photosConfig();
    const roster = authorCounts(allPhotos(), mymap.photoAuthors, config);
    const here = new Map(
      authorCounts(inView, mymap.photoAuthors, config).map((a) => [a.id, a.count]));
    authorsEl.innerHTML = "";
    ...
    for (const person of roster) {
      ...
      chip.onclick = () => {
        // Built from the roster rather than from the incoming set, so an id a
        // hand-edited URL invented cannot outlive the first click: everybody
        // on is everybody the pane can draw, and that is what collapses the
        // filter back to null.
        const next = new Set(roster.filter((p) => !allowed || allowed.has(p.id))
          .map((p) => p.id));
        if (next.has(person.id)) next.delete(person.id);
        else next.add(person.id);
        setAuthorFilter(mymap, next.size === roster.length ? null : next);
  ...
  function refresh() {
    ...
    found.forEach(({ photo, dimmed }, i) => {
      ...
      image.loading = "lazy";
      image.alt = "";
      // The same colour the pin's badge wears. The shape differs -- a row is a
      // wide thumbnail in a list and a bar reads cleanly along it, where the
      // pin needed the colour on the thing the eye already goes to -- but the
      // colour is what makes a row and its pin the same photo twice.
      const color = authorColor(mymap.photoAuthors, photo.by, config);
      if (color) {
  ...
  mymap.on("dimviewready", () => {
    if (!seeded) {
      seeded = true;
      // Before `apply()`, because the filter decides which pins get drawn and
      // the pane is not the only thing reading it.
      mymap.photoAuthorFilter = hashAuthorFilter(mymap.hashObj);
      if (persistEl) {
