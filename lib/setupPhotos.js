...
export function setupPhotoPanel(mymap, ensureLayerLoaded = async () => {}) {
   ...
   *
   * The filter counts over this rather than over what is on screen, so its
   * chips and their numbers hold still while you scrub and pan. A filter whose
  ...
  function renderAuthors() {
    if (!authorsEl) return;
    const people = authorCounts(allPhotos(), mymap.photoAuthors);
    authorsEl.innerHTML = "";
    authorsEl.hidden = people.length < 2;
    ...
    for (const person of people) {
      ...
      chip.querySelector(".photos-author-count").textContent = person.count;
      chip.onclick = () => {
        const next = new Set(allowed || people.map((p) => p.id));
        if (next.has(person.id)) next.delete(person.id);
        else next.add(person.id);
        mymap.photoAuthorFilter = next.size === people.length ? null : next;
  ...
  function refresh() {
    ...
    for (const { data, dataLayer } of mymap.photoLayers || []) {
      ...
      const selected = selectPhotos(data.photos, timeline.photoDate, mode);
      for (const { photo, dimmed } of filterByAuthor(selected, filter)) {
