...
async function load() {
  vi.resetModules();
  stubLeaflet();
  document.body.innerHTML = `<div id="photos-status"></div><div id="photos-list"></div>`;
...
describe("photos pane", () => {
  ...
  it("points at the layers tab when no photo layer is on", async () => {
    const mod = await load();
    const mymap = stubMap({ date: "20240101" });
    mymap.hasLayer = () => false;
    mod.setupPhotoPanel(mymap);
    mod.renderPhotos(mymap, LAYER, stubDataLayer());
    mymap.sidebar.fire("content", { id: "photos" });
    expect(document.getElementById("photos-status").textContent).toContain("レイヤー");
  });
});
