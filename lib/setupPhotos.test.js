...
import { authorColor } from "./photos.js";
...
function stubMap(
  { dim = "o", date = "20230101", exact = false, fill = false, photoAuthors } = {}
) {
  const listeners = {};
  const mymap = {
    dim,
    dimData: {
      ...
      // What `build-data.mjs` hoists out of every layer file in every
      // dimension, so the viewer knows the whole cast before fetching one.
...
describe("who switches photos on", () => {
  ...
  it("ticks itself for an incoming permalink that says photos stay up", async () => {
    const { mymap, isOn } = await panel();
    mymap.hashObj.pv = 1;
  ...
  it("ticks itself for an old link that spelled it as a layer id", async () => {
    // Before `pv` the state rode `v` as the photo layer's id. Links with it in
    // the wild have to keep opening the map they described.
    const { mymap, isOn } = await panel();
  ...
  it("writes the setting into the hash a permalink is built from", async () => {
    const { mymap } = await panel();
    const persist = document.getElementById("photos-persist");
    persist.checked = true;
    await persist.onchange();
    expect(mymap.hashObj.pv).toBe(1);
    persist.checked = false;
    await persist.onchange();
    expect("pv" in mymap.hashObj).toBe(false);
  });
...
describe("a permalink to one photo", () => {
  ...
  it("publishes the setting, never the photo layer's id", async () => {
    // The whole point of `pv`: a link says "photos stay up", not "layer 102 is
    // on". The id is a build-assigned number and no part of the interface.
    const mymap = await copyFromCluster();
    mymap.dimData.visibleLayers.add(102);
    mymap.hashObj.pv = 1;
    document.getElementById("photo-lightbox-link").click();
    await Promise.resolve();
    ...
    expect(hash.pv).toBe(1);
    expect(hash.dD.o.v).not.toContain(102);
  });

  it("keeps the other visible layers it always carried", async () => {
    const mymap = await copyFromCluster();
    mymap.dimData.visibleLayers.add(1);
    mymap.dimData.visibleLayers.add(102);
    document.getElementById("photo-lightbox-link").click();
    await Promise.resolve();
    await Promise.resolve();
    expect(copiedHash().dD.o.v).toEqual([1]);
  });
...
describe("arriving on a photo permalink", () => {
  ...
  async function arrive(ph, { layer = LAYER, hash = {}, visible = false } = {}) {
    const mod = await load();
    const mymap = stubMap({ date: "20240101" });
    // A link with no `ph` only shows pins if the sender had them up, which is
    // what `v` in the hash says and what this stands in for.
    if (visible) mymap.dimData.visibleLayers.add(102);
    const dataLayer = stubDataLayer();
    let onMap = false;
    ...
    mymap.hasLayer = () => onMap;
    mymap.hashObj.ph = ph;
    Object.assign(mymap.hashObj, hash);
  ...
  it("arrives filtered when the hash says so", async () => {
    // 鹿's two photos share a position and pupu's is far away, so a filter to
    // 鹿 alone is one pin instead of two.
    const { mymap } = await arrive(undefined, { layer: CREDITED, hash: { by: [2] }, visible: true });
    expect(mymap.photoAuthorFilter).toEqual(new Set([2]));
    expect(created.markers).toHaveLength(1);
  });

  it("lets a named photo through a filter that would hide it", async () => {
    // The one collision between the two things a link can carry. `find`
    // searches the pins that were drawn, so without this the link would land
    // on nothing and do nothing at all.
    const { mymap } = await arrive("a", { layer: CREDITED, hash: { by: [1] } });
    expect(document.getElementById("photo-lightbox").hidden).toBe(false);
    // By the smallest margin: 鹿 is switched on, pupu stays on, and the map is
    // still the one that was sent.
    expect(mymap.photoAuthorFilter).toEqual(new Set([1, 2]));
  });

  it("opens unfiltered when the hash asks for something that is not a filter", async () => {
    const { mymap } = await arrive(undefined, { layer: CREDITED, hash: { by: "3,1" }, visible: true });
    expect(mymap.photoAuthorFilter).toBe(null);
    expect(created.markers).toHaveLength(2);
  });

  it("takes an empty list as every author switched off", async () => {
    const { mymap } = await arrive(undefined, { layer: CREDITED, hash: { by: [] }, visible: true });
    expect(mymap.photoAuthorFilter).toEqual(new Set());
...
describe("crediting the photographer", () => {
  ...
  const fillOf = (marker) =>
    marker.getElement().querySelector(".photo-pin-count-author")
      .style.getPropertyValue("--author-fill").trim();

  it("puts the colour of the photos it holds on the count badge", async () => {
    await openPane();
    const mixed = created.markers.find((m) => m.icon.html.includes("photo-pin-count"));
    // Both of that pin's photos are 鹿's, so a flat colour and no wheel.
    expect(fillOf(mixed)).toBe(authorColor(AUTHORS, 2));
  });

  it("gives a lone photo the same colour as a dot, in the badge's place", async () => {
    // The one case a count cannot cover: no stack, and still an author.
    await openPane();
    const lone = created.markers.find((m) => !m.icon.html.includes("photo-pin-count"));
    const dot = lone.getElement().querySelector(".photo-pin-author-dot");
    expect(dot.style.backgroundColor).toBe("rgb(77, 157, 224)");   // pupu
  });

  it("splits the badge in proportion when a cluster merges two people", async () => {
    const layer = {
      ...CREDITED,
      photos: [
        { ...photo("20220101", "a", [100, 64, 200]), by: 2 },
    ...
    // Merging never has to pick a winner: the wheel says two thirds and one,
    // from twelve o'clock, and the wedges meet with no gap between them.
    expect(fillOf(created.markers[0])).toBe(
      "conic-gradient(from -90deg, #4d9de0 0.00% 33.33%, #e05d9e 33.33% 100.00%)");
  });

  it("runs a badge's wedges in the same order as the chips", async () => {
    // The chips are the badge's legend, so they start at the same place.
    globalThis.photosConfig = { authorOrder: [2, 1] };
    const layer = {
      ...CREDITED,
      photos: [
        { ...photo("20220101", "a", [100, 64, 200]), by: 1 },
        { ...photo("20220102", "b", [100, 64, 200]), by: 2 },
      ],
    };
    await openPane(layer);
    // 鹿 is id 2 and would sit second by id; the stated order moves them first.
    expect(fillOf(created.markers[0])).toBe(
      "conic-gradient(from -90deg, #e05d9e 0.00% 50.00%, #4d9de0 50.00% 100.00%)");
  });

  it("wears the colour site.json names for an author", async () => {
    globalThis.photosConfig = { authorColors: { 2: "#43c59e" } };
    await openPane();
    const mixed = created.markers.find((m) => m.icon.html.includes("photo-pin-count"));
    expect(fillOf(mixed)).toBe("#43c59e");
    // And the rows in the pane, which have to agree with the pin.
    const rows = [...document.querySelectorAll(".photos-item-author")]
      .map((i) => i.style.backgroundColor);
    expect(rows).toContain("rgb(67, 197, 158)");
  });

  it("leaves the badge red where there is nobody to tell apart", async () => {
    // The invariant the whole scheme rests on: a data repo whose photos are all
    // one person's gets the pin it had before any of this existed.
    const alone = {
      ...LAYER,
      authors: { 1: { name: "pupu" } },
    ...
    const badge = created.markers.find((m) => m.icon.html.includes("photo-pin-count"))
      .getElement().querySelector(".photo-pin-count");
    expect(badge.classList.contains("photo-pin-count-author")).toBe(false);
    expect(document.querySelector(".photo-pin-author-dot")).toBe(null);
  });
  ...
  it("draws nothing at all for a map with one contributor", async () => {
    ...
    await openPane(alone);
    expect(document.querySelector(".photo-pin-count-author")).toBe(null);
  ...
  it("keeps an author's colour where their dimension credits only them", async () => {
    // The end lists two of the three people the overworld does. Read from the
    // layer alone, a dimension one person happens to own would drop every
    // accent -- and get them back once the visitor had been somewhere else and
    // returned, because the roster had grown behind them. The build states the
    // whole cast on every dimension so the answer cannot depend on the route.
    const solo = {
      ...LAYER,
      authors: { 2: { name: "鹿" } },
      photos: LAYER.photos.map((p) => ({ ...p, by: 2 })),
    };
    await openPane(solo, { date: "20240101", photoAuthors: AUTHORS });
    const lone = created.markers.find((m) => !m.icon.html.includes("photo-pin-count"));
    expect(lone.getElement().querySelector(".photo-pin-author-dot")
      .style.backgroundColor).toBe("rgb(224, 93, 158)");   // 鹿, id 2
    expect(document.querySelector(".photos-item-credit").textContent).toBe("鹿");
  });

  it("still draws nothing where the whole site is one person", async () => {
    // The gate itself is unchanged: the union is what it reads, and a site with
    // one contributor has nothing to tell apart in any dimension.
    const solo = {
      ...LAYER,
      authors: { 1: { name: "pupu" } },
      photos: LAYER.photos.map((p) => ({ ...p, by: 1 })),
    };
    await openPane(solo, { date: "20240101", photoAuthors: { 1: { name: "pupu" } } });
    expect(document.querySelector(".photo-pin-author-dot")).toBe(null);
    expect(document.querySelector(".photos-item-credit")).toBe(null);
  });
...
describe("the author filter", () => {
  ...
  it("puts the chips in the order site.json asked for", async () => {
    // The one thing that outranks share of the dimension: a site that has
    // stated an order gets it, in the pane and on every pin.
    globalThis.photosConfig = { authorOrder: [1, 2] };
    await openPane();
    expect(chips().map((c) => c.querySelector(".photos-author-name").textContent))
      .toEqual(["pupu", "鹿"]);
  });

  it("writes the filter into the hash the permalink is built from", async () => {
    const mymap = await openPane();
    chips().find((c) => c.textContent.startsWith("鹿")).click();
    expect(mymap.hashObj.by).toEqual([1]);
    // And everybody on again is no filter at all, so an untouched map carries
    // nothing and a link from it is the link it always was.
    chips().find((c) => c.textContent.startsWith("鹿")).click();
    expect(mymap.hashObj.by).toBeUndefined();
  });
