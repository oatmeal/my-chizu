...
export async function setupTimelinePanel(mymap) {
  ...
  function initTimelinePanel(dates) {
    ...
    // Photo counts per date, as the build emits them: a plain date-keyed
    // object, so a tile date's own row can quote its count without the panel
    // ever loading a layer file full of individual photos.
    const photoCounts = mymap.dimData.photoDates || {};
    ...
    function addDateEntry(date) {
      ...
      // The date's own photo count, rendered as the `(📷n)` the month headings
      // already use. Without it the only rows advertising photos were the
      // photo-only ones, which reads as though a day with tiles never has any.
      text.innerHTML = groupSummaryHtml({
        prefix: format(date),
        counts: { photo: photoCounts[date] || 0 },
      });
    ...
    const photoEntries = Object.entries(photoCounts)
