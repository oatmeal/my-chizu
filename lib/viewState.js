...
export function initTimeline(dimData, hashDimData) {
  ...
  // Photos filter on their own date so that a date with photos but no tiles can
  // still be selected: that row sets `photoDate` to itself and `date` to the
  // nearest earlier tile date, so the terrain shows what was there when the
  // photo was taken. Every other path keeps the two equal.
  if (dimData.timeline.photoDate === undefined) {
    dimData.timeline.photoDate = hh?.p ?? dimData.timeline.date;
  }
}
